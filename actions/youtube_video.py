...
def _get_chrome_profile() -> str:
    try:
        config_path = BASE_DIR / "config" / "api_keys.json"
        with open(config_path, "r", encoding="utf-8") as f:
            return json.load(f).get("chrome_profile", "Default")
    except Exception:
        return "Default"
...
def open_browser(url: str = None) -> bool:
    ...
    Returns True if launched via exe, False if fallback.
    """
    import shutil
    import platform
    ...
    if browser_exe:
        if browser_exe.lower() == "chrome" and platform.system() == "Windows":
            chrome_path = _get_chrome_path()
            if chrome_path:
                try:
                    profile = _get_chrome_profile()
                    args = [chrome_path, f"--profile-directory={profile}"]
                    if url:
                        args.append(url)
                    subprocess.Popen(args)
                    time.sleep(2.5)
                    print(f"[YouTube] ✅ Opened browser via direct path: {args}")
                    return True
                except Exception as e:
                    print(f"[YouTube] ⚠️ Direct path start failed: {e}")

        exe_path = shutil.which(browser_exe) or shutil.which(browser_exe + ".exe")
        if exe_path:
            try:
                args = [exe_path]
                if browser_exe.lower() == "chrome":
                    profile = _get_chrome_profile()
                ...
                subprocess.Popen(args)
                time.sleep(2.5)
                print(f"[YouTube] ✅ Opened browser via exe: {args}")
                return True
            except Exception as e:
...
def _handle_play(parameters: dict, player) -> str:
    ...
    launched_direct = open_browser(url)
