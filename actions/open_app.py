    ...
    if app_name.lower() == "chrome":
        ...
        if chrome_path:
            try:
                ...
                print(f"[open_app] Launching Chrome directly via path with profile: {profile}")
                subprocess.Popen([chrome_path, f"--profile-directory={profile}"])
                time.sleep(2.0)
                return True
            except Exception as e:
                print(f"[open_app] Chrome direct launch failed: {e}, falling back to Windows Search")
