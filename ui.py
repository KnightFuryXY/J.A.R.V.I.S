...
VERSION = "v1.0.1"
LOCAL_BUILD_DATE = "2026-04-05T14:24:17Z"
...
class JarvisUI:
    def __init__(self, face_path, size=None):
        ...
        self.FACE_SZ = min(int(H * 0.44), 360)
        self.FCX     = W // 2
        self.FCY     = int(H * 0.12) + self.FACE_SZ // 2
    ...
    def _draw(self):
        ...
        c.create_text(W - 16, 31, text=time.strftime("%I:%M:%S %p"),
                      fill=C_PRI, font=("Courier", 14, "bold"), anchor="e")

        # ── Durum göstergesi ──────────────────────────────────────────────────
        log_y_pos = H - 190
        available_gap = log_y_pos - (FCY + FW // 2)
        sy = FCY + FW // 2 + max(20, available_gap // 2 - 15)
        ...
        # Sürüm numarası sol tarafta
        v_text = f"{VERSION} (UPDATE AVAILABLE)" if self.update_available else f"{VERSION} (LATEST)"
        v_col = C_ACC2 if self.update_available else C_DIM
        c.create_text(16, H - 14, fill=v_col, font=("Courier", 8),
                      text=v_text, anchor="w")

        c.create_text(W // 2, H - 14, fill=C_DIM, font=("Courier", 8),
                      text="Huzefa.Creative  ·  CLASSIFIED  ·  MARK XXXV")
    ...
    def _api_keys_exist(self):
        ...
        try:
            with open(API_FILE, "r", encoding="utf-8") as f:
                ...
                return bool(key and not key.startswith("AIzaSyAwCxvjqE7JXh3050YC0TRtVmVreepb9mE") and "YOUR" not in key)
        except Exception:
            return False
    ...
    def _check_for_updates(self):
        try:
            ...
            req = urllib.request.Request(
                "https://api.github.com/repos/KnightFuryXY/J.A.R.V.I.S/commits/main",
                headers={"User-Agent": "Jarvis-Update-Checker"}
            )
            with urllib.request.urlopen(req, timeout=5) as response:
                data = json.loads(response.read().decode())
                latest_date = data["commit"]["committer"]["date"]
                if latest_date > LOCAL_BUILD_DATE:
